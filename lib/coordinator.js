...
Coordinator.prototype.addDoc = function (docID) {
	...
	// Model's data
	this.models[docID] = new Model();
	this.models[docID].docID = docID;
	this.models[docID].ropes = new LogootSRopes();
	this.models[docID].replicaNumber = 0;
	this.models[docID].creationDate = new Date();
	this.models[docID].lastModificationDate = new Date();
	this.models[docID].lastConnexionDate = new Date();
	this.models[docID].history = [];
...
Coordinator.prototype.deleteDoc = function (docID) {
	Model.remove({ docID: docID }, function (err) {
		if (err) {
...
Coordinator.prototype.join = function (args) {
	...
	if(this.models[args.docID] === null || this.models[args.docID] === undefined) {
		...
		Model.findOne({ docID: new RegExp('^'+args.docID+'$') }, function (err, model) {
			if (err) {
				return handleError(err);
			}
			// Add it to the data structure
			coordinator.models[args.docID] = model;
			temp = new LogootSRopes();
			temp.copyFromJSON(coordinator.models[args.docID].ropes);
			coordinator.models[args.docID].ropes = temp;
			coordinator.models[args.docID].bufferLogootSOp = [];
			coordinator.models[args.docID].flag = false;
			coordinator.models[args.docID].infosUsers = {};
			coordinator.models[args.docID].infosUsersUpdated = false;
			coordinator.models[args.docID].nbConnected = 0;
		...
		temp = [];
		Utils.pushAll(temp, model.bufferLogootSOp);
		Utils.pushAll(model.bufferLogootSOp, args.bufferLocalLogootSOp);
		args.bufferLogootSOp = temp;
		delete args.bufferLocalLogootSOp;

		console.log('En résumé:');
		console.log('temp: ', temp);
		console.log('model.bufferLogootSOp: ', model.bufferLogootSOp);
...
Coordinator.prototype.cleanBufferLogootSOp = function (docID) {
	...
	if(model.flag !== true) {
		...
		while(model.bufferLogootSOp.length > 0) {
	        ...
	        if(OK) {
	        	console.log('On traite: ', lo);
		        tos = lo.execute(model.ropes);
		        console.log('Ca génère: ', tos);
