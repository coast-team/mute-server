...
var SocketIOAdapter = function (server, coordinator, infosUsersModule, delay) {
	var network = this;
	...
	this.io.sockets.on('connection', function (socket) {
	  	...
	  	socket.on('joinDoc', function (data, callback) {
	  		var temp = [];
	  		Utils.pushAll(temp, data.bufferLocalLogootSOp);
			socket.room = data.docID;
			socket.join(socket.room);
			network.join(socket, data);
			callback({ error: false, length: temp.length });
			setTimeout(function () {
	  			network.io.sockets.in(socket.room).emit('broadcastOps', { logootSOperations: temp });
	  		}, network.delay + 750);
	  	});
	...
	this.coordinator.on('doc', function (args) {
		...
		infosUsersModule.addUser(args.socket.room, args.replicaNumber);
	...
	this.coordinator.on('infosUsers', function (data) {
		...
		network.io.sockets.in(data.docID).emit('sendInfosUsers', msg);
	});
};
