...
var SocketIOAdapter = function (server, coordinator) {
	...
	io.sockets.on('connection', function (socket) {
	  	...
	  	socket.on('operation', function (data, callback) {
			...
			socket.broadcast.to(socket.room).emit('operation', data);
			network.emit('receive', data);
			callback({ error: false, length: data.logootSOperations.length});
	  	});

		socket.on('disconnect', function () {
			console.log('On a bien une déconnexion ^^');
	...
	this.coordinator.on('join', function (args) {
		var message = {
			"ropes": args.ropes,
