...
Coordinator.prototype.addDoc = function (docID) {
	...
	this.models[docID].creationDate = new Date().toLocaleString();
	this.models[docID].lastModificationDate = new Date().toLocaleString();
...
Coordinator.prototype.join = function (args) {
	...
	model.nbConnected++;
	model.lastConnexionDate = new Date().toLocaleString();
...
Coordinator.prototype.cleanBufferLogootSOp = function (docID) {
	...
	if(model.flag != true) {
		...
		while(model.bufferLogootSOp.length > 0) {
		    ...
		    model.lastModificationDate = new Date().toUTCString();
		}
		model.flag = false;
	}
};
